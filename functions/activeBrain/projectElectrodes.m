...
disp(sprintf('Projecting electrodes:'));
...
%compute the distance of all vertices from the electrode being processed
       disp(sprintf('           computing the distance from the electrode for all vertices'));
       dvect = zeros(1, Vv);
...
%normal vector computation (computation of line points p1 and p2):
       disp(sprintf('           computing the normal vector'));
       ...
       %compute the normal vector
           ...
           %close(gcf);
           normals=vertexNormal(triangulation(M.tri,M.vert(:, 1), M.vert(:, 2), M.vert(:, 3)));
...
%compute close vertices
       ...
       if ~isempty(interstype),
           disp(sprintf('           finding near vertices'));
           ...
           disp(sprintf('           sorting the vertices'));
           ...
           sorttri=[];
           Lcv = length(sortvert);
           disp(sprintf('           finding near triangles'));
           for k = 1 : Lcv,
               cv = sortvert(k);
...
%compute the intersection with the triangles
       disp(sprintf('           computing the intersection with the triangles'));
       Lct = length(sorttri);
       inside = 0;
