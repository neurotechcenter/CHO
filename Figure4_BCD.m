...
for snr_iter = 1:length(osc_amp_list)
    ...
    for iter = 1:n_iteration
        ...
        [ACC{snr_iter,iter} SENS{snr_iter,iter} SPEC{snr_iter,iter} SNR{snr_iter,iter}] = harmonic_osc_simulation_for_different_method_v8(sim_param);
    end
end

%
save('./data/HS_simulation_MSNR_test_v9.mat', 'ACC', 'SENS','SPEC','SNR');


%%
load HS_simulation_MSNR_test_v9.mat

SNR_data = [];

for snr_iter = 1:length(osc_amp_list)
    for iter = 1:n_iteration
        SNR_data = [SNR_data SNR{snr_iter,iter}]; 
    end
end
