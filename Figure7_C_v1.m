...
n_subjects = 8;
load ach_data_final.mat

load x_ecog_rt_data.mat;
...
%     subjects = [06 08 10 12 14 15];    
    
        load(['s' num2str(isub) '_cho_method_allperiod_v1.mat']);
        
    rt_data = subj_rt_data{isub};
    ...
    for ch = ach_data{isub} %1:n_ch
        
        if 1 %isBRODMANN(ch,SecondaryLabel,[44 45])
            for tr = 1:n_trials
...
%                     hist_data = [hist_data; center_fp (stop_time-start_time)/srate*1000];
                    hist_data = [hist_data; center_fp (start_time)/srate*1000];
                end
...
%         else
    ...
    all_rt_data = [all_rt_data rt_data];
...
hdata([1 2 3 61]) = 0;
% hist(hist_data(:,2),x_axis);
bar(x_axis,hdata,'FaceColor', [0 0.5 0.5]);
[mv maxi_hist] = max(hdata);
...
subplot(3,3, [8 9]);
hdata = hist(all_rt_data*1000,x_axis);
hdata([1 2 3 61]) = 0;
% hist(hist_data(:,2),x_axis);
bar(x_axis,hdata,'FaceColor', [0 0.5 0.5]);
[mv maxi_hist] = max(hdata);
